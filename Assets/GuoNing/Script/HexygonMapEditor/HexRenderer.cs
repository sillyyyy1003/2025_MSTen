...
public struct Face
{
   public List<Vector3> vertices { get; private set; }  // 頂点
   public List<int> triangles { get; private set; } // 三角形インデックス
   public List<Vector2> uvs { get; private set; }  // UV座標
...
public class HexRenderer : MonoBehaviour
{
	...
	protected Vector3 GetPoint(float size, float _height, int index)
	{
		float angle_deg = isFlatTopped ? 60 * index : 60 * index - 30;
        float angle_rad = Mathf.PI/180f * angle_deg;
        return new Vector3((size * Mathf.Cos(angle_rad)), _height, size * Mathf.Sin(angle_rad));
	}
